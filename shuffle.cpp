                   ...
                   compute::context &context)
{
    ...
    std::vector<float> host_out(128);
    std::vector<int> host_rand(128);
...
//?????????????????????????????????????????????????????????????
//    compute::uniform_int_distribution<int> dist1(0,128);
//    for (int i=0; i<128; ++i)
//        host_rand[i] =  dist();
//?????????????????????????????????????????????????????????????
    ...
    compute::copy(d_input.begin(), d_input.end(), host_out.begin(), queue);
